...
function greetingTest() {
	...
	//Check the current time to decide on which greeting to return
	if (currentHour < 12) {
...
function player_Win() {
	...
	else if (Gameplay.points >= 50) {
		update_display_msg(19);
	}
	else {
...
function cmd_Inventory_check() {
	...
	for (i = 0, len = player_inventory.length; i < len; i++) {
		if (player_inventory[i].inventory_q != 0) {
			inv_list = capitaliseFirstLetter(player_inventory[i].inventory_name).toString() + " x " + player_inventory[i].inventory_q.toString() + "\n" + inv_list;
		}
	}
	if (inv_list != "") {
...
function cmd_19() {
	if (current_loc === 8) {
		...
		setInterval(function() {player_Lose()}, 1500);
	}
}
